/*---------------------------------------------------------------------------*
  ...
  MIi_SetMainMemoryPriority を MI_～に
  ...
  ARM7 では、MI_SetMainMemoryPriority(), MI_SetCardProcessor(),
  MI_SetCartridgeProcessor() 自体を定義しないようにした。
  ...
  ARM7側の EXMEMCNT_L、EXMEMCNT_H 記述に併せた変更
...
#define REG_MI_EXMEMCNT_MP_B_SHIFT   REG_MI_EXMEMCNT_L_MP_B_SHIFT
#define REG_MI_EXMEMCNT_MP_B_SIZE    REG_MI_EXMEMCNT_L_MP_B_SIZE
#define REG_MI_EXMEMCNT_MP_B_MASK    REG_MI_EXMEMCNT_L_MP_B_MASK
...
static inline void MIi_SetExCardProcessor(MIProcessor proc)
{
    MI_PROCESSOR_ASSERT(proc);
    reg_MI_EXMEMCNT =
        (u16)((reg_MI_EXMEMCNT & ~REG_MI_EXMEMCNT_MP_B_MASK) | (proc << REG_MI_EXMEMCNT_MP_B_SHIFT));
}
#endif
...
{
    return (MIProcessor)((reg_MI_EXMEMCNT & REG_MI_EXMEMCNT_MP_B_MASK) >> REG_MI_EXMEMCNT_MP_B_SHIFT);
}
